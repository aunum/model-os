...
class Client(Kind):
    ...
    def __init__(
        ...
        if server is not None:
            ...
            self.uri = server.store_cls(dev_dependencies=dev_dependencies, clean=clean, hot=hot, repo=repo)
...
class Object(Kind):
    ...
    @classmethod
    def _extract_and_load(cls: Type[O]) -> Type[O]:
        cls_package = cls._cls_pkg()
        # If the process is a notebook, we extract the class into a separate file and then use it
        fp = extract_cls_to_file(cls)

        if cls_package == "__main__":
            mod_name = path_to_module(fp)
            spec = importlib.util.spec_from_file_location(mod_name, fp)
            if spec is None:
                raise ValueError("Trouble loading extracted object class, if you hit this please open an issue", fp)
            mod = importlib.util.module_from_spec(spec)
            sys.modules[mod_name] = mod
            if spec.loader is None:
                raise ValueError(
                    "Trouble loading extracted object class, loader is None, if you hit this please open an issue",
                    fp,  # noqa
                )
            spec.loader.exec_module(mod)

        else:
            mod = importlib.import_module(f".{extracted_file_name(cls)}", package=cls_package)  # noqa: F841

        loc_copy = locals().copy()
        ret = eval(f"mod.{cls.__name__}", loc_copy)
        return ret

    @classmethod
    def _cls_pkg(cls) -> str:
        modl = cls.__module__
        msplit = modl.split(".")
        if len(msplit) > 1:
            msplit = msplit[:-1]
...
if __name__ == "__main__":
    ...
    {fin_sig}
        ...
        _sock = socket.create_connection(
            (f"{{self.pod_name}}.pod.{{self.pod_namespace}}.kubernetes", int(self.server_port)))
{param_ser_lines_joined}
...
{ret_ser_lines_joined}
        ...
        if len(main_imports) == 0:
            main_imports_joined = "pass"
        else:
